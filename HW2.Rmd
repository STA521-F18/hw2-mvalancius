...
Our response variable of interest, ModernC, has approximately linear marginal relationships with Change and Purban. THe plot between fertility and ModernC shows some curvature. The marginal relationship between ModernC and Change is approximately linear and negatively correlated. Without controlling for other variables, areas with lower rates of population growth have higher levels of modern contraception use. Knowing that areas of high growth also general tend to have lower wealth/education could lead to speculation that the relationship between change and PPgdp is important in predicting ModernC.

The relationship of ModernC with PPgdp appears to be better modeled with a curve, suggesting a logarithmic or power transformation might better approximate a linear relationship, while the relationship with population is unclear do to some potential outliers. From the density plot of population, we see that it exhibits a heavy positive skew, with several countries (especially China and India) having much larger populations that the median. Consequentially, a logarithmic transformation of population could bring these orders of magnitude into scale.

In addition to checking for a linear relationship between the response and the individual predictors, in a multivariate setting, a good general procedure is to transform the predicters so that they exhibit a close to linear relationship. Many of the predictors plotted against each other have relationships that do not appear linear, and thus utilizing transformations is appropriate. We will further inverstigate the predictor relationships after applying the transformations.

While the appropriate transformations will be discussed further in a later question, I wanted to plot the pairs after transforming PPgdp, Fertility, and Population logarithmically. Justification for these transformations is graphical. Population has a distribution that spans many orders of magnitude, while PPgdp and Fertility span several orders of magnitude and have non linear relationships with ModernC based on the plotted samples.
...
With these transformations, most of the relationships appear more linear. From a marginal perspective, all of the variables except for log(Pop) have a relationship with MordernC. It is worth noting that, on its own, Frate does not have much of a relationship with any of the other predictors individually.

Additionally, many of the plots of predictors now demonstrate a closer to linear relationship, suggesting that further transformations may be uneccesary.
...
plot(UN3_mod_simple)
```
In a multiple linear regression setting, interpretations of residual plots are dependent upon there being a somewhat linear relationship among the predictors. Since this model was fit without any transformations, that assumption is questionable, at best. So even though the residuals display a roughly constant variance with no clear patterns, interpreting the residuals vs pitted plots is inconclusive. Thus, although the Q-Q plot strongly suggests that the residuals do not follow a theoretical normal distribution, we need to transform the variables and re-run the model to make stronger conclusions about the its fit.
...
Looking at the added variabke plots, we can see the amount of variation unexplained by the model compared to the variation in the added variable that is not explained by the other variables. 
...
As seen, China and India both have significantly smaller predicted values when the model was trained with their observations. This is evidence that the inclusion of these two observations, which have population values far larger than any other population values, greatly influence the fit of the model. 

This is also seen clearly in the added variable plots. The left plot below is the added variable plot with China and India included, while the plot to the right excludes them.
...
```{r, warning = FALSE}
par(mfrow = c(1,2))
ggplot(UN3, aes(x = Pop, y = ModernC)) + geom_point() + geom_smooth(method = "lm", se = FALSE) + geom_smooth(method = "loess", se = FALSE)

ggplot(UN3, aes(x = log(Pop), y = ModernC)) + geom_point() + geom_smooth(method = "lm", se = FALSE) + geom_smooth(method = "loess", se = FALSE)
par(mfrow = c(1,1))
```
Comparing the plots of ModernC against population and then the log of population, we see that the transformation better models the majority of the data, although the relationship is not very strong. The flexible loess regression line offers a comparison to the linear regression line and demonstrates the stark contrast between the fit of the two plots. 

 (2) Fertility
 
```{r, warning = FALSE}
par(mfrow = c(1,3))
ggplot(UN3, aes(x = Fertility, y = ModernC)) + geom_point() + geom_smooth(method = "lm", se = FALSE) + geom_smooth(method = "loess", se = FALSE)

ggplot(UN3, aes(x = log(Fertility), y = ModernC)) + geom_point() + geom_smooth(method = "lm", se = FALSE) + geom_smooth(method = "loess", se = FALSE)

ggplot(UN3, aes(x = (Fertility)^(-1), y = ModernC)) + geom_point() + geom_smooth(method = "lm", se = FALSE) + geom_smooth(method = "loess", se = FALSE)
par(mfrow = c(1,1))
```
 
WHile the log transformation improves the linear relationship, there is still some curvature for lower values of fertility. Checking the Box-Tidwell results could recommend a more suitable transformation.

(3) PPgdp
```{r, warning = FALSE}
par(mfrow = c(1,2))
ggplot(UN3, aes(x = PPgdp, y = ModernC)) + geom_point() + geom_smooth(method = "lm", se = FALSE) + geom_smooth(method = "loess", se = FALSE)

ggplot(UN3, aes(x = log(PPgdp), y = ModernC)) + geom_point() + geom_smooth(method = "lm", se = FALSE) + geom_smooth(method = "loess", se = FALSE)

par(mfrow = c(1,1))

```
As the above plots demonstrate, before the transformation, most of the PPgdp values are clustered below 200, but there are several that are orders of magnitude greater than the smallest. Thus, it is a natural candidate for a logarithmic transformation. The second plot demonstrates that the transformation produces a much closer to linear relationship,


While the three logarithmic transformations appear to resolve the lack of linearity in the predictors from a graphical analysis, it is useful to compare these with the BoxTidwell transformations.
...
```
None of the maximum likelihood estimates for the powers are statistically significant at the 0.05 level. Without a strong statistical or scientific justification for the transformations and due to the improvement in the linear relationships with the log transformation, the latter transformations are preferred, 
...
plot(UN3_mod_tran)
```
Since the variables with the transformations have approximately linear relationships with eachother, we can now analyze the residual plots. In the Fitted Values vs. Standardized Residuals plot, we see no noticable trend in the residuals and the variance appears constant. However, the Q-Q plot does give some concern that the residuals are not quite normally distributed, with the thin tails indivating that the residuals are more tightly centered around the 0.
...
```
Using a model that includes all the predictors with no transformations, we see that the Box-Cox method recommends transforming the response variable to the power of around 0.8. the 95% confidence interval for the estimate contains 1 (no transformation). Because of this and the difficulties in justifying a power of 0.8 theoretically, the optimal procedure is to leave the predictor unchanged. Thus, both when we transformed the variables and when we didn't, the Box_cox method provided evidence that a a power transformation of the response variable, ModernC, would not be optimal.
...
summary(UN3_mod_tran)
```
From the summary, we see that the slope of Chane is 2.31.
...
```
Thus, we have confirmed that the slope in the added variable plot is the same as the slope in the full model.
